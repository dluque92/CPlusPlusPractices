...
<table class="listado">
		...
		<c:forEach items="${listadoProfesores}" var="profesor">
			<tr>
				...
				<c:forEach items="${listadoAsignaturas}" var="asignatura">
					<c:if test="${asignatura.idAsignatura == profesor.idAsignatura}">
						<td align="center">${asignatura.nombre}</td>
					</c:if>
				</c:forEach>
				<td align="center">
					<table class="vacio">
						<tr>
	                		...
	                		<td>
				                <form name="borrar${profesor.idProfesor}" action="<%= Constantes.URLSERVIDOR %>" method="post"> 
